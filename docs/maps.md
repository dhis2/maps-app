...
Overlay layers are represented by additional layer _cards_ in the layer panel such as:
...
click on the row to open the Feature Profile:
...
You will enter a "download mode" where you can adjust the map layout before you download the image.
...
-   **Show map name**: Select if you want to include the map name or not. This option is only
    available if the map is saved. To change the name, exit download mode and select _File_ >
    _Rename_.
-   **Show map description**: Select if you want to include the map description or not. This option
    ...
    description, exit download mode and select _File_ > _Rename_.
...
Map download is not supported in Internet Explorer or Safari. We recommend using Google Chrome or
Firefox.
