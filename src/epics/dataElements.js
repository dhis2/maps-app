...
export const loadDataElementOperands = (action$) =>
    action$
        ...
        .concatMap((action) =>
            getD2()
                .then(d2 => apiFetch(`/dataElementOperands.json?fields=id,${getDisplayPropertyUrl(d2)}&paging=false&filter=dataElement.dataElementGroups.id:eq:${action.groupId}`))
                .then(data => setDataElementOperands(action.groupId, data.dataElementOperands))
        );
