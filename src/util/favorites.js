...
export const cleanMapConfig = ({
    ...
    // An untouched combinedTableRef placeholder gets cleaned up. Only when
    // cleanMapviewConfig is true — otherwise (e.g. FileMenu's rename-only
    // save) combinedJoinConfig is still packed inside the unparsed config
    // JSON string rather than lifted to this flat field.
    mapViews: (cleanMapviewConfig
        ? config.mapViews.filter(
              (view) =>
